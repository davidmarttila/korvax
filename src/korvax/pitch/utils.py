...
def midi_to_hz(
    notes: Float[Array, "*axes"], a4_hz: float = 440.0, a4_midi: float = 69.0
) -> Float[Array, "*axes"]:
    return a4_hz * (2.0 ** (notes - a4_midi) / 12.0)


def hz_to_midi(
    frequencies: Float[Array, "*axes"], a4_hz: float = 440.0, a4_midi: float = 69.0
) -> Float[Array, "*axes"]:
...
def hz_to_cents(
    frequencies: Float[Array, "*axes"], hz_ref: float
) -> Float[Array, "*axes"]:
